...
open class AudioPlayerManager: NSObject {
	...
	/// Set this to true if the `AudioPlayerManager` print should be enabled. The default is `false`.
	public static var verbose						= true

	/// Set this to true if the `AudioPlayerManager` print should contain detailed information about the calling class, function and line. The default is `true`
	public static var detailedprint					= true
	...
	open func play(_ audioTrack: AudioTrack) {
		print("play(audioTrack: \(audioTrack))")
		self.stop()
		self.play([audioTrack], at: 0)
	}

	open func play(_ audioTracks: [AudioTrack], at startIndex: Int) {
		print("play(audioTracks: \(audioTracks.count) tracks, startIndex: \(startIndex))")
	...
	open func replace(with audioTrack: AudioTrack) {
		print("replace(audioTrack: \(audioTrack))")
		self.replace(with: [audioTrack], at: 0)
	}

	open func replace(with audioTracks: [AudioTrack], at startIndex: Int) {
		print("replace(audioTracks: \(audioTracks.count) tracks, startIndex: \(startIndex))")
		self.stop()
	...
	open func prepend(_ audioTracks: [AudioTrack], toQueue generation: Int) {
		if (self.queueGeneration == generation) {
			print("Queue generation (\(generation)) gets prepended with \(audioTracks.count) tracks.")
			self.queue.prepend(audioTracks)
			self.callPlayStateChangeCallbacks()
		} else {
			print("Queue generation (\(generation)) isn't the same as the current (\(self.queueGeneration)). Won't prepend \(audioTracks.count) tracks.")
		}
	}

	open func append(_ audioTracks: [AudioTrack], toQueue generation: Int) {
		if (self.queueGeneration == generation) {
			print("Queue generation (\(generation)) gets appended with \(audioTracks.count) tracks.")
			self.queue.append(audioTracks)
			self.callPlayStateChangeCallbacks()
		} else {
			print("Queue generation (\(generation)) isn't the same as the current (\(self.queueGeneration)). Won't append \(audioTracks.count) tracks.")
		}
	}
