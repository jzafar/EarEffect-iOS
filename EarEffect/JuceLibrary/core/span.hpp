...
#define TCB_SPAN_NAMESPACE_NAME am
...
inline void contract_violation(char const* msg) { throw contract_violation_error(msg); }

#elif defined(TCB_SPAN_TERMINATE_ON_CONTRACT_VIOLATION)
[[noreturn]] inline void contract_violation(char const* /*unused*/) { std::terminate(); }
#endif
...
public:
    ...
    template<typename T, std::size_t N, std::size_t E = Extent,
             typename std::enable_if<
                 (E == dynamic_extent || N == E)
                     && detail::is_container_element_type_compatible<std::array<T, N> const&, ElementType>::value,
                 int>::type
             = 0>
    TCB_SPAN_ARRAY_CONSTEXPR span(std::array<T, N> const& arr) noexcept : storage_(arr.data(), N)
    {
    }
    ...
    template<typename Container, std::size_t E = Extent,
             typename std::enable_if<
                 ...
                 int>::type
             = 0>
    constexpr span(Container const& cont) : storage_(detail::data(cont), detail::size(cont))
    {
    }
    ...
    template<
        ...
        typename std::enable_if<(Extent == dynamic_extent || OtherExtent == dynamic_extent || Extent == OtherExtent)
                                ...
                                int>::type
        = 0>
    constexpr span(span<OtherElementType, OtherExtent> const& other) noexcept : storage_(other.data(), other.size())
    {
    }
...
template<typename T, std::size_t N>
TCB_SPAN_ARRAY_CONSTEXPR span<const T, N> make_span(std::array<T, N> const& arr) noexcept
{
    return {arr};
}
...
template<typename Container>
constexpr span<const typename Container::value_type> make_span(Container const& cont)
{
    return {cont};
}
